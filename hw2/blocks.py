...
class Linear(Block):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.normal(torch.zeros(size=(out_features, in_features)), wstd)
...
class CrossEntropyLoss(Block):
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        dx = torch.sum(torch.exp(x), dim=1, keepdim=True).pow_(-1) * torch.exp(x)
...
class Sequential(Block):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        out = reduce(lambda res, block: block.forward(res, **kw), self.blocks, x)
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        din = reduce(lambda grad, block: block.backward(grad), reversed(self.blocks), dout)
        # ========================
    ...
    def params(self):
        ...
        # ====== YOUR CODE: ======
        params = chain.from_iterable((block.params() for block in self.blocks))
        params = list(params)
        # ========================
...
class MLP(Block):
    ...
    def __init__(
        ...
        num_prev_layer_out_features = in_features
