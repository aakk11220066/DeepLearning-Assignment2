...
class LeakyReLU(Block):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = torch.where(x>0, torch.Tensor([1]), torch.Tensor([self.alpha])) * dout
        # ========================
...
class TanH(Block):
    ...
    def forward(self, x, **kw):
        ...
        #  Save whatever you need into grad_cache.
        # ====== YOUR CODE: ======
        out = 1-(2*torch.exp(-x))/(torch.exp(x)+torch.exp(-x))
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        x = self.grad_cache["x"]
        dx = 4*dout/(torch.square(torch.exp(x)+torch.exp(-x)))
        # ========================
...
class Linear(Block):
    ...
    def forward(self, x, **kw):
        ...
        #out = torch.mm(x, torch.transpose(self.w, 0, 1)) + self.b
    ...
    def backward(self, dout):
        ...
        self.dw += torch.mm(torch.transpose(dout, 0, 1), x)
...
class CrossEntropyLoss(Block):
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        exp_x = torch.exp(x)
        dx = exp_x/torch.sum(exp_x, dim=1, keepdim=True)
...
class Dropout(Block):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:
            prob = torch.rand(*x.shape)<self.p
            out = torch.where(prob, torch.Tensor([0]), x)
    ...
    def backward(self, dout):
        ...
        dx = None
        if self.training_mode:
            prob = self.grad_cache["prob"]
            dx = torch.where(prob, torch.Tensor([0]), dout)
        else:
            dx = dout*(1-self.p)
        # ========================
...
class MLP(Block):
    ...
    def __init__(
        ...
        **kw,
        ...
        blocks.append(Linear(num_classes, num_classes))'''
        activate = ReLU if activation=="relu" else Sigmoid
        input_features = in_features
        for i in hidden_features:
            blocks.append(Linear(input_features, i))
            input_features = i
            blocks.append(activate())
            if dropout!=0:
                blocks.append(Dropout(dropout))
        blocks.append(Linear(input_features, num_classes))
        # ========================
