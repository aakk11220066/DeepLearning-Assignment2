    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            #    argument.
            # ====== YOUR CODE: ======
            res = self.train_epoch(dl_train, verbose=verbose, **kw)
            losses, accuracy = res.losses, res.accuracy
            train_loss.extend(losses)
            #train_loss.append(sum(losses)/len(losses))
            train_acc.append(accuracy)

            res = self.test_epoch(dl_test, verbose=verbose, **kw)
            losses, accuracy = res.losses, res.accuracy
            test_loss.extend(losses)
            #test_loss.append(sum(losses) / len(losses))
            test_acc.append(accuracy)

            if early_stopping:
                if len(test_acc)>1 and accuracy<=test_acc[-2]:
                    break

            if not checkpoints is None:
                torch.save(self.model, checkpoints+".pt")
            # ========================
...
class BlocksTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        scores = self.model(torch.flatten(X, start_dim=1))
